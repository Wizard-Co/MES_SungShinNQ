...
namespace WizMes_SungShinNQ
{
    ...
    public partial class Win_Qul_CustomDefect_U : UserControl
    {
        ...
        private void btnCustomer_InGroupBox_Click(object sender, RoutedEventArgs e)
        {
            ...
            //AFT 에서 굳이 거래처별 품명만 검색하지 말것! 20190422
        ...
        // 품명 키다운 _ 품번 찾기로 변경 요청 2020.03.23, 장가빈
        ...
        // 플러스파인더 _ 품명찾기 -> 품번찾기로 변경 2020.03.23, 장가빈
        private void btnArticle_InGroupBox_Click(object sender, RoutedEventArgs e)
        {
            pf.ReturnCode(txtArticle_InGroupBox, 84, txtArticle_InGroupBox.Text);
            ...
            //}
            //else { pf.ReturnCode(txtArticle_InGroupBox, 1, ""); }


            //if (txtArticle_InGroupBox.Tag.ToString().Length > 0)
            //{
            //    txtArticleID_InGroupBox.Text = txtArticle_InGroupBox.Tag.ToString();
            //}
        ...
        private void FillGrid()
        {
            ...
            try
            {
                ...
                sqlParameter.Add("nchkArticleID", 0);// nchkArticleID);              // 인트
        ...
        private void dgdCustomDefect_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (ViewReceiver != null)
            {
                ...
                //#region 날짜놀이
                //// 날짜 놀이. _ 기존 데이터 저장이 엉망인 케이스가 있어서ㅓ..  이런 노가다가 조금 필요한...ㅠㅠ.
                ...
                //if ((ViewReceiver.ReplyReqDate != string.Empty) && (ViewReceiver.ReplyReqDate != null) && (lib.IsNullOrWhiteSpace(ViewReceiver.ReplyReqDate) != true))     //회신요청일
                //{
                //    dtpReplyReqDate.Text = DateTime.ParseExact(ViewReceiver.ReplyReqDate, "yyyyMMdd", null).ToString("yyyy-MM-dd");
                //}
                //else { dtpReplyReqDate.Text = string.Empty; }

                //if ((ViewReceiver.NotifyDate != string.Empty) && (ViewReceiver.NotifyDate != null) && (lib.IsNullOrWhiteSpace(ViewReceiver.NotifyDate) != true))     //통보일
                //{
                //    dtpNotifyDate.Text = DateTime.ParseExact(ViewReceiver.NotifyDate, "yyyyMMdd", null).ToString("yyyy-MM-dd");
                //}
                //else { dtpNotifyDate.Text = string.Empty; }

                //if ((ViewReceiver.ReplyDate != string.Empty) && (ViewReceiver.ReplyDate != null) && (lib.IsNullOrWhiteSpace(ViewReceiver.ReplyDate) != true))     //회신(응답)일
                //{
                //    dtpReplyDate.Text = DateTime.ParseExact(ViewReceiver.ReplyDate, "yyyyMMdd", null).ToString("yyyy-MM-dd");
                //}
                //else { dtpReplyDate.Text = string.Empty; }

                //if ((ViewReceiver.AcptDate != string.Empty) && (ViewReceiver.AcptDate != null) && (lib.IsNullOrWhiteSpace(ViewReceiver.AcptDate) != true))     //접수일
                //{
                //    dtpAcptDate.Text = DateTime.ParseExact(ViewReceiver.AcptDate, "yyyyMMdd", null).ToString("yyyy-MM-dd");
                //}
                //else { dtpAcptDate.Text = string.Empty; }

                //if ((ViewReceiver.DefectRespectDate != string.Empty) && (ViewReceiver.DefectRespectDate != null) && (lib.IsNullOrWhiteSpace(ViewReceiver.DefectRespectDate) != true))     //불량접수 검토일
                //{
                //    dtpDefectRespectDate.Text = DateTime.ParseExact(ViewReceiver.DefectRespectDate, "yyyyMMdd", null).ToString("yyyy-MM-dd");
                //}
                //else { dtpDefectRespectDate.Text = string.Empty; }

                //if ((ViewReceiver.CorrExpectDate != string.Empty) && (ViewReceiver.CorrExpectDate != null) && (lib.IsNullOrWhiteSpace(ViewReceiver.CorrExpectDate) != true))     //조치 예정일
                //{
                //    dtpCorrExpectDate.Text = DateTime.ParseExact(ViewReceiver.CorrExpectDate, "yyyyMMdd", null).ToString("yyyy-MM-dd");
                //}
                //else { dtpCorrExpectDate.Text = string.Empty; }

                //if ((ViewReceiver.CorrCompDate != string.Empty) && (ViewReceiver.CorrCompDate != null) && (lib.IsNullOrWhiteSpace(ViewReceiver.CorrCompDate) != true))     //조치 완료일
                //{
                //    dtpCorrCompDate.Text = DateTime.ParseExact(ViewReceiver.CorrCompDate, "yyyyMMdd", null).ToString("yyyy-MM-dd");
                //}
                //else { dtpCorrCompDate.Text = string.Empty; }

                //if ((ViewReceiver.CorrRespectDate != string.Empty) && (ViewReceiver.CorrRespectDate != null) && (lib.IsNullOrWhiteSpace(ViewReceiver.CorrRespectDate) != true))     //시정조치 검토일
                //{
                //    dtpCorrRespectDate.Text = DateTime.ParseExact(ViewReceiver.CorrRespectDate, "yyyyMMdd", null).ToString("yyyy-MM-dd");
                //}
                //else { dtpCorrRespectDate.Text = string.Empty; }

                //if ((ViewReceiver.CorrProdFromDate != string.Empty) && (ViewReceiver.CorrProdFromDate != null) && (lib.IsNullOrWhiteSpace(ViewReceiver.CorrProdFromDate) != true))     //시정조치 생산 시작일
                //{
                //    dtpCorrProdFromDate.Text = DateTime.ParseExact(ViewReceiver.CorrProdFromDate, "yyyyMMdd", null).ToString("yyyy-MM-dd");
                //}
                //else { dtpCorrProdFromDate.Text = string.Empty; }

                //if ((ViewReceiver.CorrProdToDate != string.Empty) && (ViewReceiver.CorrProdToDate != null) && (lib.IsNullOrWhiteSpace(ViewReceiver.CorrProdToDate) != true))     //시정조치 생산 마지막일
                //{
                //    dtpCorrProdToDate.Text = DateTime.ParseExact(ViewReceiver.CorrProdToDate, "yyyyMMdd", null).ToString("yyyy-MM-dd");
                //}
                //else { dtpCorrProdToDate.Text = string.Empty; }
