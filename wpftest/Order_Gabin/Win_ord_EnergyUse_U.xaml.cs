...
namespace WizMes_SungShinNQ
{
    ...
    public partial class Win_ord_EnergyUse_U : UserControl
    {
        ...
        {
            //DataStore.Instance.InsertLogByFormS(this.GetType().Name, stDate, stTime);
        ...
        private void FillGrid()
        {
            ...
            try
            {
                ...
                sqlParameter.Add("sCodeID", chkEnergySearch.IsChecked == true ? cboEnergySearch.SelectedValue : "");

                //DataSet ds = DataStore.Instance.ProcedureToDataSet("xp_Ord_Energy_sEnergy", sqlParameter, false);
                DataSet ds = DataStore.Instance.ProcedureToDataSet("xp_Ord_Energy_sEnergy", sqlParameter, true);
                ...
                if (ds != null && ds.Tables.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        foreach (DataRow dr in drc)
                        {
                            ...
                            if (chkEnergySearch.IsChecked == true)
                            {
                            ...
                            {
                                EnergySum.gbnEnergy = "전체";
                                EnergySum.UnitEnergy = "";
                            }

                            //총합계 콤마찍기
                            result1 += ConvertDouble(Energy.cstElectQty); 
                            EnergySum.cstElectQtySum = stringFormatN0(result1);
                            result2 += ConvertDouble(Energy.cstElectAmount);
                            EnergySum.cstElectAmountSum = stringFormatN0(result2);
        ...
        private void FillChartGraph(DataGrid dataGrid)
        {
            try
            {
                ...
                ChartInfoEnergy chartEnergy = new ChartInfoEnergy();
                chartEnergy.seriesCollection = new SeriesCollection();
                chartEnergy.EnergyQty = new ChartValues<double>();
                chartEnergy.Labels = new string[dataGrid.Items.Count];

                int index = 0;
                for (int i = 0; i < dataGrid.Items.Count; i++)
                {
                    var Energy = dataGrid.Items[i] as Win_ord_EnergyUse_U_CodeView2;


                    if (Energy != null)
                    {
                        chartEnergy.Labels[index] = Energy.CstYYYYMM;
                        index++;

                        if (Energy.cstElectQty != null
                            && CheckConvertDouble(Energy.cstElectQty))
                        {
                            chartEnergy.EnergyQty.Add(ConvertDouble(Energy.cstElectQty));
                        }
                        else
                        {
                            chartEnergy.EnergyQty.Add(0);
                        }

                    }
                }

                chartEnergy.seriesCollection.Add(new ColumnSeries
                {
                    Values = chartEnergy.EnergyQty,
                    //StackMode = StackMode.Values,
                    DataLabels = true,
                    Title = "이용량"
                });

                chartEnergy.Formatter = value => value + "Kwh";
                this.DataContext = chartEnergy;
            }
            catch (Exception ex)
            {
        ...
        {
            Regex regex = new Regex("[^0-9.-]+");
            //.Handled = regex.IsMatch(e.Text);
            if (regex.IsMatch(e.Text))
            {
                e.Handled = true; // 이벤트 처리를 막습니다.
            }
            else
            {
                e.Handled = false; // 이벤트 처리를 허용합니다.
            }
    ...
    { 
        public string cstElectQtySum { get; set; }
        public string cstElectAmountSum { get; set; }
    ...
    class ChartInfoEnergy
    {
        public SeriesCollection seriesCollection { get; set; }
        public string[] Labels { get; set; }
        public Func<double, string> Formatter { get; set; }
        public ColumnSeries columnSeries { get; set; }
        public ChartValues<double> EnergyQty { get; set; }
