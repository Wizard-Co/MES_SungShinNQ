...
using WizMes_SungShinNQ.Properties;
...
namespace WizMes_SungShinNQ
{
    ...
    public class DataStore
    {
        ...
        public DataSet QueryToDataSetSafe(string queryString, Dictionary<string, object> parameters = null)
        {
            try
            {
                if (p_Connection.State == ConnectionState.Closed)
            ...
            {
                // throw e; 보다는 원본 스택 트레이스 보존
                throw;
            }
            finally
            {
                p_Command.Parameters.Clear(); // 파라미터 정리
            }
        }
...
//#endif
        ...
        public string[] ExecuteAllProcedureOutputNew(List<Procedure> AllProcedure, List<Dictionary<string, object>> sqlParameterall)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < AllProcedure.Count; i++)
                {
                    var procedure = AllProcedure[i];
                    var sqlParameter = sqlParameterall[i];
                    ...
                    if (procedure.OutputUseYN == "Y")
                    {
                        outputValues[procedure.OutputName] = "";
                    }

                    // 프로시저 설정
                    p_Command.CommandText = procedure.Name;
                    p_Command.CommandType = CommandType.StoredProcedure;
                    p_Command.Parameters.Clear();

                    // 파라미터 설정
                    SetParameters(sqlParameter, outputValues);
                    ...
                    // com_TxnLog 테이블에 로그 삽입(호출메서드, 넘겨주는 파라미터, 호출 프로그램이름)
                    InsertTrxLogByUserID(procedure.Name, sqlParameter);
                    ...
                    if (procedure.OutputUseYN == "Y")
                    {
                        outputValues[procedure.OutputName] = p_Command.Parameters[procedure.OutputName].Value?.ToString() ?? "";
                    }
                }

                transaction.Commit();
                return new string[] { "success", lastReturnValue };
            }
            catch (NullReferenceException)
            {
                transaction?.Commit();
        ...
        // 파라미터 설정 로직을 별도 메서드로 분리 (중복 제거)
        private void SetParameters(Dictionary<string, object> sqlParameter, Dictionary<string, string> outputValues)
        {
            foreach (var kvp in sqlParameter)
            {
                object paramValue = kvp.Value;

                if (outputValues.ContainsKey(kvp.Key) && !string.IsNullOrEmpty(outputValues[kvp.Key]))
                {
                    paramValue = outputValues[kvp.Key];
                }

                p_Command.Parameters.AddWithValue(kvp.Key, paramValue);
            }
        }
        ...
        private void InsertTrxLogByUserID(string procedureName, Dictionary<string, object> parameters)
        {
            try
            {
                ...
                //        DataStore.Log_Instance.ExecuteProcedureByErrorLog("xp_iLog", sqlParameter, false); //이거 서강에 프로시저가 없음..
                //    }
