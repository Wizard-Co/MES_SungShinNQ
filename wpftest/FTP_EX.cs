	...
	public class FTP_EX
	{
		...
		public bool createDirectory(string newDirectory)
		{
            ...
            catch (WebException ex)
            {
                ...
                {
                    /* Make path if the foldes are not exists at all */
                    string parentDirectory = Path.GetDirectoryName(newDirectory.Replace('\\', '/'));
                    /* Make the parent folders in the path */
                    if (createParentDirectories())
                    {
                        /*If every folders were made in success, make the last one */
        ...
        //폴더가 삭제되거나 미리 만들지 않았을 경우 대비
        //상위폴더가 없을 때 상위폴더를 재귀적으로 먼저 만들고 하위폴더(PK Value)로 만든다.        
        private bool createParentDirectories()
        {
            string basePath = host.Split(new[] { "//" }, StringSplitOptions.None)[1];
            string[] pathParts = basePath.Split('/');

            /* Check the Port Number */
            string hostPart = pathParts[0].Split(':')[0];
            /* Bring the Port Number if exists */
            string portPart = basePath.Contains(":") ? basePath.Split(':')[1].Split('/')[0] : "";

            /* Set the path to make the folders */
            string currentPath = $"ftp://{hostPart}";
            if (!string.IsNullOrEmpty(portPart))
            {
                currentPath += ":" + portPart;
            }

            /* Make the folders in recursive */
            for (int i = 1; i < pathParts.Length; i++)
            {
                ...
                try
                {
                    ...
                    {
                        /* If folders are already exists, do nothing. */
                    }
                }
                catch (WebException ex)
                {
