...
namespace WizMes_SungShinNQ
{
    ...
    public partial class Win_prd_Eval_Q : UserControl
    {
        ...
        private void btnMCEvalBasis_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (mvm.Menu.Equals("설비 등급 평가기준 등록"))
                {
                    break;
                }
                k++;
            ...
            {
                Type type = Type.GetType("WizMes_SungShinNQ." + MainWindow.mMenulist[k].ProgramID.Trim(), true);
                object uie = Activator.CreateInstance(type);

                MainWindow.mMenulist[k].subProgramID = new MdiChild()
                {
                    Title = "SungShinNQ [" + MainWindow.mMenulist[k].MenuID.Trim() + "] " + MainWindow.mMenulist[k].Menu.Trim() + 
