    ...
    public partial class Win_prd_RunningRate_Q : UserControl
    {
        ...
        private void FillGrid()
        {
            try
            {
                ...
                sqlParameter.Add("chkProcessID", chkProcess.IsChecked == true && !string.IsNullOrWhiteSpace(cboProcess.SelectedValue.ToString()) ? 1 : 0);
                sqlParameter.Add("ProcessID", ProcessID);

                sqlParameter.Add("chkMachineID", chkMachine.IsChecked == true && !string.IsNullOrWhiteSpace(cboMachine.SelectedValue.ToString()) ? 1 : 0);
                sqlParameter.Add("MachineID", MachineID);

                DataSet ds = DataStore.Instance.ProcedureToDataSet("xp_prd_sMCRunningRate_WPF", sqlParameter, false); //2022-09-27 기본시간 8 ->10.5시간으로 변경 요청 여영애
                if (ds != null && ds.Tables.Count > 0)
                {
                    DataTable dt = ds.Tables[0];
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow dr in drc)
                        {
                            ...
                            var dgdWorkRate = new Win_prd_RunningRate_Q_CodeView()
                            {
                                ...
                                DayNonWorkHour = stringFormatN2(dr["DayNonWorkHour"]),
