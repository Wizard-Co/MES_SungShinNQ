...
namespace WizMes_SungShinNQ
{
    ...
    public partial class Win_mtr_Outware_Q : UserControl
    {
        ...
        private void FillGrid()
        {
            ...
            sqlParameter.Add("BuyerArticleNo", chkArticle.IsChecked == true && !txtArticle.Text.Trim().Equals("") ? txtArticle.Text : ""); //@escape함수제거
                    ...
                    foreach (DataRow item in drc)
                    {
                        if (item["Depth"].ToString() == "0")
                        {
                            var Window_SubulOutwareViewInsert = new Win_mtr_Outware_QView()
                            {
                                ...
                                UnitPrice = stringFormatN1(item["UnitPrice"]),
